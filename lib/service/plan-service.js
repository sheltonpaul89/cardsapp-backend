...
async function createPlan(plan) {
    try {
        if (plan.recurrence_duration < 0 || plan.allowed_requests < 0 || plan.plan_cost < 0)
            eh.throwException(Errors.InvalidPlanValues);
        let existing_plans = await this.getPlans({recurrence_duration:plan.recurrence_duration,
...
async function getPlanById(plan_id) {
    try {
        let response = await this.getPlans({id:plan_id});
        if (response == null)
