...
async function getRequests(user_id,queryParams = {}) {
    ...
    if(get_all === true)
        response = await mongodb.selectAll({servicer_user_id:user_id},collection_name);
    response = response.concat(await mongodb.selectAll({user_id:user_id},collection_name));    
    return response;
}
