...
async function GetUserByParam(request, reply) {
    try {
        if(request.query.phone || request.query.email)
            return await UserService.searchUser(request.query);
        ExceptionHandler.throwException(Errors.UserSearchException, request.query.phone | request.query.email);
    } catch (error) {
        let err = ExceptionHandler.processException(error);
        return reply.response(err).code(err.statusCode);
    }
...
async function sendOTP(request, reply) {
    try {
        if(request.payload.phone_number)
            ...
            return await OTPService.sendOTPEmail(request.payload.email);
        ExceptionHandler.throwException(Errors.OTPInputException);
    } catch (error) {
        let err = ExceptionHandler.processException(error);
        return reply.response(err).code(err.statusCode);
    }
...
async function verifyOTP(request, reply) {
    try {
        if(request.payload.phone_number)
            ...
            return await OTPService.verifyOTP(request.payload.email,request.payload.otp);
        ExceptionHandler.throwException(Errors.OTPInputException);
    } catch (error) {
        let err = ExceptionHandler.processException(error);
        return reply.response(err).code(err.statusCode);
